...
import { processShopping } from './shoppingAgent.js';
import { processNews, isNewsRelated } from './newsAgent.js';
import {
  OrchestratorContext,
  AgentResponse
} from '../types/index.js';
...
반드시 JSON만 출력하세요.`;
  ...
  private async handleMCPIntent(
    ...
    switch (intent) {
      ...
      case 'news':
        return await this.handleNews(userMessage, extractedInfo);
  ...
  private async handleStandardIntent(
    ...
    switch (routerResult.intent) {
      ...
      case 'shopping':
        // ShoppingAgent로 처리 (MCP 패턴)
        return await processShopping(standardResult, agentContext);
  ...
  private async handlePlaceRecommendation(info: any): Promise<AgentResponse> {
    ...
    // 간략한 텍스트 메시지 (상세 정보는 카드로 표시)
    const areaName = info.area || '해당 지역';
    const topRated = restaurants[0];
    const message = `**${areaName}** 인기 맛집 ${Math.min(restaurants.length, 9)}곳을 찾았어요! 🍽️\n\n가장 추천하는 곳은 **${topRated.name}**이에요${topRated.rating ? ` (⭐ ${topRated.rating})` : ''}. 아래 카드에서 더 자세한 정보를 확인해보세요!`;

    return {
      message,
      suggestions: restaurants.slice(0, 3).map((r: any) => r.name),
      mcp_data: { restaurants: restaurants.slice(0, 9) }  // 최대 9개
    };
  }
   ...
   * 쇼핑/상품 검색 처리
   *
   * @연결점: ShoppingAgent (shoppingAgent.ts)를 사용하여 처리
   * - MCP 패턴 기반 상품 추천
   * - 사용자 목적 적합도 기준 정렬 (특정 브랜드 우선 X)
   */
  private async handleShopping(info: any): Promise<AgentResponse> {
    // ShoppingAgent 사용 (MCP 패턴)
    const agentContext = {
      userId: this.context.user_id,
    ...
    const routerResult = {
      extractedInfo: {
        searchQuery: info.searchQuery || info.title,
      ...
      originalMessage: info.originalMessage || info.searchQuery || ''
    };

    return await processShopping(routerResult, agentContext);
  }
   ...
   * 뉴스 브리핑 처리
   */
  private async handleNews(userMessage: string, info: any): Promise<AgentResponse> {
    const agentContext = {
      userId: this.context.user_id,
      today: new Date().toISOString().split('T')[0]
    };
    ...
    return await processNews(routerResult, agentContext);
  }
  ...
  private async handleComplexScenario(
    ...
    if (requiredTools.includes('maps_recommend_restaurants') && info.area) {
      ...
      if (placeResult.mcp_data?.restaurants) {
        const count = placeResult.mcp_data.restaurants.length;
        message += `🍽️ **${info.area}** 맛집 ${count}곳을 찾았어요!\n\n`;
    ...
    if (requiredTools.includes('shopping_search') || requiredTools.includes('shopping_goal_recommendations')) {
      ...
      if (shoppingResult.mcp_data?.products) {
        const count = shoppingResult.mcp_data.products.length;
        message += `🛒 관련 상품 ${count}개를 찾았어요!\n\n`;
